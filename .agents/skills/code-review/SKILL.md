...
| `tsc --noEmit` in `../openwop-sdks/sdk/typescript/` and `../openwop/conformance/` | **ZERO** errors — BLOCKING |
| `redocly lint ../openwop/api/openapi.yaml` | Clean — BLOCKING |
| `asyncapi validate ../openwop/api/asyncapi.yaml` | Clean — BLOCKING |
| `bash ../openwop/scripts/check-security-invariants.sh` | Every MUST-NOT has a public test — BLOCKING |
| `bash ../openwop/scripts/openwop-check-publish-metadata.sh` | No placeholder URLs, stale module paths — BLOCKING for release |
| `bash ../openwop/scripts/check-npm-pack-contents.sh` | No package content leaks — BLOCKING for release |
| `@ts-ignore` / `@ts-expect-error` | **BANNED** in `../openwop-sdks/sdk/typescript/src/`, `../openwop/conformance/src/` (test files included unless documented justification) |
| `@ts-nocheck` | **BANNED** in production code; allowed only in ../openwop/conformance/SDK test files with documented justification |
...
**Search for in `../openwop-sdks/sdk/typescript/src/`, `../openwop/conformance/src/lib/`, `../openwop-examples/examples/hosts/*/src/`:**
...
**Search for in `../openwop-sdks/sdk/typescript/src/__tests__/`, `../openwop/conformance/src/scenarios/`, `../openwop-examples/examples/hosts/*/test/`:**
...
**Search for in `../openwop/api/openapi.yaml` and `../openwop/api/asyncapi.yaml`:**
...
Per `../openwop/SECURITY/invariants.yaml` and `../openwop/scripts/check-security-invariants.sh`:

- Every protocol-tier MUST-NOT has at least one public test in `../openwop/conformance/src/scenarios/`.
...
- New fixture: added to `../openwop/conformance/fixtures.md` catalog + per-fixture contracts; `spec-corpus-validity.test.ts` round-trip will fail otherwise
...
- Cross-references use relative paths (`./capabilities.md` from inside `../openwop/spec/v1/`, `./spec/v1/capabilities.md` from repo root)
...
- Agent packs (per `../openwop/RFCS/0003`) validate against `agent-manifest.schema.json`
...
- ROADMAP entry added/checked if the change closes a known gap (`../openwop/docs/PROTOCOL-GAP-CLOSURE-PLAN.md` tracks)
...
1. [WIRE-SHAPE] **../openwop/schemas/run-event.schema.json:42 — existing required field made optional**
...
2. [CONFORMANCE-GATING] **../openwop/conformance/src/scenarios/new-surface.test.ts:1 — scenario runs unconditionally**
   - Issue: New optional surface needs to gate on `host.newSurface.supported` per ../openwop/conformance/coverage.md §"Capability-gated scenarios"
...
3. [SDK-CONTRACT] **../openwop-sdks/sdk/typescript/src/client.ts:120 — new endpoint missing in `OpenwopClient`**
   - Issue: ../openwop/api/openapi.yaml gained `/v1/runs/{runId}:newOp` but no SDK method exists
...
4. [RFC-2119] **../openwop/spec/v1/<doc>.md §New section — uses lowercase "should"**
...
| `../openwop/conformance/src/` | `as any` / `@ts-ignore` | 0 required |
| `../openwop/schemas/` | Missing `additionalProperties: false` | 0 required |
| `api/` | Inline schema (no `$ref`) | 0 required |
| `../openwop/spec/v1/` + `../openwop/RFCS/` | Lowercase normative imperatives | 0 required |
