 ...
 * `GET /v1/host/openwop-app/example-data/status`, `POST .../demo/run`, `POST .../demo/clear`.
 *
...
const base = `${config.baseUrl}/v1/host/openwop-app/example-data`;
...
export async function getExampleDataStatus(): Promise<ExampleDataStep[]> {
  const res = await fetch(`${base}/status`, fetchOpts({ headers: authedHeaders() }));
  return (await asJson<{ steps: ExampleDataStep[] }>(res, 'getExampleDataStatus')).steps ?? [];
}
