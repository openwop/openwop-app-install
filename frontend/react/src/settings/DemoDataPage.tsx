/**
 * `/demo-data` (Settings → Demo data) — the demo-seeding dashboard.
 *
 * Renders one row per demo data type the backend's seeder registry reports
 ...
 * registry, so a new demo data type appears here with zero changes to this file.
 *
...
export function DemoDataPage(): JSX.Element {
  ...
  const onSeed = async (all: boolean) => {
    ...
    try {
      ...
      const r = await runDemoSeed({ ...(stepIds ? { steps: stepIds } : {}), dryRun });
      setResult(r);
  ...
  const onClear = async () => {
    const ids = [...selected];
    const label = ids.length ? ids.join(', ') : 'all demo data';
    if (!window.confirm(`Clear ${label}? This removes the demo entities for your tenant (your own agents are not touched).`)) return;
  ...
  return (
    <section>
      <PageHeader
        eyebrow="Settings"
        title="Demo data"
        lede="Load sample data so the dashboards have something to show — agents, workforces, and their history. Everything here is explicit and clearly demo; a clean install starts empty."
      />

      {error ? <Notice variant="error">{error}</Notice> : null}

      <div className="surface-card u-mt-3">
        <h2 className="u-fs-16 u-mt-0">Demo data types</h2>
        ...
        {steps === null ? (
          ...
          <StateCard icon={<DatabaseIcon />} title="No demo data types registered" body="This host advertises no seedable demo data." />
        ) : (
        ...
        <div className="action-bar u-gap-3 u-wrap u-mt-3 u-items-center">
          ...
          <button type="button" className="btn-accent-solid" disabled={busy !== null || (steps?.length ?? 0) === 0} onClick={() => void onSeed(true)}>
            <DatabaseIcon size={14} /> {busy === 'seed' ? 'Loading…' : 'Load all demo data'}
          </button>
          ...
          <button type="button" className="secondary" disabled={busy !== null} onClick={() => void onClear()} title="Remove demo entities (your own agents are untouched)">
            <TrashIcon size={14} /> {busy === 'clear' ? 'Clearing…' : 'Clear demo data'}
          </button>
        </div>
