/**
 * Sharing (host-extension product feature — ADR 0013).
 *
 * Gates on useFeatureAccess('sharing'). An org picker → a "create a share link"
 * form (resource type → resource → optional label + expiry) → the org's active
 * links with copyable PUBLIC /shared/:token URLs + revoke. The link resolves a
 * read-only view of the resource publicly (incl. a CMS DRAFT — the preview-link
 * use-case the published-only public surface can't serve).
 */
import { useCallback, useEffect, useState } from 'react';
import { PageHeader } from '../../ui/PageHeader.js';
import { Notice } from '../../ui/Notice.js';
import { StateCard } from '../../ui/StateCard.js';
import { Skeleton } from '../../ui/Skeleton.js';
import { toast } from '../../ui/toast.js';
import { useFeatureAccess } from '../../featureToggles/FeatureAccessContext.js';
import { GlobeIcon, LinkIcon, LockIcon, PlusIcon, TrashIcon } from '../../ui/icons/index.js';
import {
  createLink,
  listLinks,
  listOrgs,
  listResources,
  revokeLink,
  sharedUrl,
  type Org,
  type ResourceRef,
  type ResourceType,
  type ShareLink,
} from './sharingClient.js';

const TYPE_LABEL: Record<ResourceType, string> = { cms_page: 'CMS page', kb_collection: 'KB collection' };

function copy(text: string): void {
  void navigator.clipboard?.writeText(text).then(() => toast.success('Link copied')).catch(() => { /* clipboard blocked */ });
}

export function SharingPage(): JSX.Element {
  const access = useFeatureAccess('sharing');
  const [orgs, setOrgs] = useState<Org[] | null>(null);
  const [orgId, setOrgId] = useState('');
  const [linkList, setLinkList] = useState<ShareLink[] | null>(null);
  const [error, setError] = useState<string | null>(null);

  // mint form
  const [resourceType, setResourceType] = useState<ResourceType>('cms_page');
  const [resources, setResources] = useState<ResourceRef[]>([]);
  const [resourceId, setResourceId] = useState('');
  const [label, setLabel] = useState('');
  const [expiry, setExpiry] = useState('');
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    if (!access.enabled) return;
    void listOrgs().then((o) => { setOrgs(o); setOrgId((cur) => cur || (o[0]?.orgId ?? '')); }).catch(() => setOrgs([]));
  }, [access.enabled]);

  const loadLinks = useCallback((org: string) => {
    void listLinks(org).then(setLinkList).catch((e) => setError(e instanceof Error ? e.message : 'Failed to load links.'));
  }, []);

  useEffect(() => { if (orgId) { setLinkList(null); loadLinks(orgId); } }, [orgId, loadLinks]);

  // Load pickable resources whenever org or type changes (guard stale resolves).
  useEffect(() => {
    if (!orgId) return;
    let active = true;
    setResourceId('');
    void listResources(orgId, resourceType).then((r) => { if (active) setResources(r); }).catch(() => { if (active) setResources([]); });
    return () => { active = false; };
  }, [orgId, resourceType]);

  const create = useCallback(async () => {
    if (!orgId || !resourceId) return;
    setBusy(true);
    try {
      const days = expiry.trim() ? Number(expiry) : undefined;
      await createLink(orgId, {
        resourceType,
        resourceId,
        ...(label.trim() ? { label: label.trim() } : {}),
        ...(days && Number.isFinite(days) ? { expiresInDays: days } : {}),
      });
      setLabel(''); setExpiry(''); setResourceId('');
      loadLinks(orgId);
      toast.success('Share link created');
    } catch (e) { toast.error(e instanceof Error ? e.message : 'Create failed.'); }
    finally { setBusy(false); }
  }, [orgId, resourceType, resourceId, label, expiry, loadLinks]);

  const revoke = useCallback(async (token: string) => {
    try { await revokeLink(orgId, token); loadLinks(orgId); }
    catch (e) { toast.error(e instanceof Error ? e.message : 'Revoke failed.'); }
  }, [orgId, loadLinks]);

  if (access.loading) return <Skeleton />;
  if (!access.enabled) {
    return <StateCard icon={<LockIcon />} title="Sharing is not enabled" body="Ask an administrator to enable the Sharing feature for this tenant." />;
  }

  const orgPicker = orgs && orgs.length > 0 ? (
    <select value={orgId} onChange={(e) => setOrgId(e.target.value)} className="u-w-auto" aria-label="Organization">
      {orgs.map((o) => <option key={o.orgId} value={o.orgId}>{o.name}</option>)}
    </select>
  ) : undefined;

  const active = (linkList ?? []).filter((l) => !l.revoked);

  return (
    <div>
      <PageHeader eyebrow="Platform" title="Sharing" lede="Mint unguessable public links to a page or knowledge collection." actions={orgPicker} />
      {error ? <Notice variant="error">{error}</Notice> : null}

      {!orgs ? <Skeleton /> : orgs.length === 0 ? (
        <StateCard icon={<GlobeIcon />} title="No organizations" body="Create an organization first — share links belong to an org." />
      ) : (
        <div className="sharing-layout">
          {/* Mint form */}
          <div className="surface-card u-gap-2">
            <strong>Create a share link</strong>
            <label className="u-label-sm">Resource type
              <select value={resourceType} onChange={(e) => setResourceType(e.target.value as ResourceType)}>
                <option value="cms_page">CMS page</option>
                <option value="kb_collection">KB collection</option>
              </select>
            </label>
            <label className="u-label-sm">Resource
              <select value={resourceId} onChange={(e) => setResourceId(e.target.value)}>
                <option value="">— select —</option>
                {resources.map((r) => <option key={r.id} value={r.id}>{r.label}</option>)}
              </select>
            </label>
            <label className="u-label-sm">Label (optional)
              <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Draft for review" />
            </label>
            <label className="u-label-sm">Expires in days (optional)
              <input value={expiry} onChange={(e) => setExpiry(e.target.value)} inputMode="numeric" placeholder="never" />
            </label>
            <div className="u-flex u-justify-end">
              <button type="button" className="btn-primary" disabled={busy || !resourceId} onClick={() => void create()}><PlusIcon /> Create link</button>
            </div>
          </div>

          {/* Active links */}
          <div className="surface-card u-gap-2">
            <strong>Active links</strong>
            {!linkList ? <Skeleton /> : active.length === 0 ? <span className="u-label-sm">No active share links.</span> : active.map((l) => (
              <div key={l.token} className="surface-inset sharing-link">
                <div className="u-flex u-gap-2 u-items-center u-wrap">
                  <span className="chip">{TYPE_LABEL[l.resourceType]}</span>
                  <strong className="sharing-link-title">{l.label ?? l.cardTitle ?? l.resourceId}</strong>
                  <span className="u-flex-1" />
                  {l.expiresAt ? <span className="u-label-sm" title={l.expiresAt}>expires {new Date(l.expiresAt).toLocaleDateString()}</span> : null}
                  <button type="button" className="btn-ghost" title="Copy public link" aria-label="Copy public link" onClick={() => copy(sharedUrl(l.token))}><LinkIcon /></button>
                  <button type="button" className="btn-ghost" title="Revoke" aria-label="Revoke link" onClick={() => void revoke(l.token)}><TrashIcon /></button>
                </div>
                <code className="sharing-link-url">{sharedUrl(l.token)}</code>
              </div>
            ))}
          </div>
        </div>
      )}
    </div>
  );
}
