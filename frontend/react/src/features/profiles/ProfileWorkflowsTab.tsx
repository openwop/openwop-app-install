...
export function ProfileWorkflowsTab({ workflows, onSaved }: { workflows: string[]; onSaved: (p: Profile) => void }): JSX.Element {
  ...
  return (
    <div>
      ...
      {workflows.length === 0 ? (
        <StateCard icon={<WorkflowIcon />} title="No workflows assigned yet" body="Assign one from the library below to build your portfolio — the work you (or your assistant) run." />
      ) : (
        <>
          <p className="muted u-fs-13 u-mt-0">
            Your workflow portfolio — the work you own. Each card explains what it does; run it now or drop a card into a
            trigger lane on <strong>My Board</strong> to fire it.
          </p>
            ...
            {workflows.map((wfId) => {
              ...
              return (
                <div key={wfId} className="surface-card">
                  ...
                  <div className="agentportfolio-purpose">{workflowPurpose(wfId) ?? (known ? '' : 'Local workflow — assigned to you.')}</div>
                  {!known ? (
