 ...
 * can't see it):
 *   - GET /v1/host/sample/kanban/boards/personal ensures + returns the caller's
...
async function signup(c: ReturnType<typeof client>): Promise<{ userId: string }> {
  const r = await c.post('/v1/host/sample/test/login', { email: `pb-${Date.now()}-${n++}@acme.test` });
...
describe('ADR 0025 — personal board route', () => {
  it('GET /boards/personal ensures + returns the caller\'s own board, idempotently', async () => {
    const c = client();
    await signup(c);
    const r = await c.get('/v1/host/sample/kanban/boards/personal');
    ...
    const r2 = await c.get('/v1/host/sample/kanban/boards/personal');
  ...
  it('the owner reaches + mutates their personal board from a DIFFERENT active workspace', async () => {
    const c = client();
    await signup(c);
    const personal = (await c.get('/v1/host/sample/kanban/boards/personal')).body.board;

    // Switch into a shared workspace → active tenant != the board's (personal) tenant.
    const ws = (await c.post('/v1/host/sample/workspaces', { name: 'Acme Corp' })).body.workspaceId;
    ...
    const again = await c.get('/v1/host/sample/kanban/boards/personal');
    ...
    const byId = await c.get(`/v1/host/sample/kanban/boards/${encodeURIComponent(personal.id)}`);
    expect(byId.status).toBe(200);

    // And the owner can WRITE to it from the shared workspace.
    const created = await c.post(`/v1/host/sample/kanban/boards/${encodeURIComponent(personal.id)}/cards`, { title: 'Ship it', columnId: 'todo' });
  ...
  it('a different user cannot reach the owner\'s personal board by id (fail-closed)', async () => {
    const owner = client();
    await signup(owner);
    const board = (await owner.get('/v1/host/sample/kanban/boards/personal')).body.board;

    const other = client();
    await signup(other);
    const r = await other.get(`/v1/host/sample/kanban/boards/${encodeURIComponent(board.id)}`);
  ...
  it('an anon sandbox session is refused a personal board (durable-only)', async () => {
    ...
    const anon = client();
    const r = await anon.get('/v1/host/sample/kanban/boards/personal');
...
describe('ADR 0025 — personal schedules (owner=me)', () => {
  it('creates a user-owned schedule, lists it via ?owner=me, and isolates it from other users', async () => {
    ...
    // No schedules yet.
    const empty = await c.get('/v1/host/sample/scheduler/jobs?owner=me');
    expect(empty.status).toBe(200);
    expect(empty.body.jobs).toEqual([]);

    // Create a personal schedule (server derives the owner from the session).
    const created = await c.post('/v1/host/sample/scheduler/jobs', { owner: 'me', cronExpr: '0 9 * * *', workflowId: 'wf.brief' });
    expect(created.status, JSON.stringify(created.body)).toBe(201);
    ...
    const ws = (await c.post('/v1/host/sample/workspaces', { name: 'Acme' })).body.workspaceId;
    await c.post(`/v1/host/sample/workspaces/${encodeURIComponent(ws)}/switch`);
    const mine = await c.get('/v1/host/sample/scheduler/jobs?owner=me');
    ...
    const paused = await c.patch(`/v1/host/sample/scheduler/jobs/${encodeURIComponent(created.body.jobId)}`, { enabled: false });
    ...
    const other = client();
    await signup(other);
    const otherList = await other.get('/v1/host/sample/scheduler/jobs?owner=me');
    expect(otherList.body.jobs.map((j: any) => j.jobId)).not.toContain(created.body.jobId);
    const otherPatch = await other.patch(`/v1/host/sample/scheduler/jobs/${encodeURIComponent(created.body.jobId)}`, { enabled: true });
  ...
  it('a re-submitted identical personal schedule is idempotent (deterministic id, no duplicate)', async () => {
    const c = client();
    await signup(c);
    const a = await c.post('/v1/host/sample/scheduler/jobs', { owner: 'me', cronExpr: '0 9 * * *', workflowId: 'wf.brief' });
    expect(a.status).toBe(201);
    // Same content again → same job id, 200 (not a new row).
    const b = await c.post('/v1/host/sample/scheduler/jobs', { owner: 'me', cronExpr: '0 9 * * *', workflowId: 'wf.brief' });
    expect(b.status).toBe(200);
    expect(b.body.jobId).toBe(a.body.jobId);
    const list = await c.get('/v1/host/sample/scheduler/jobs?owner=me');
    ...
    const d = await c.post('/v1/host/sample/scheduler/jobs', { owner: 'me', cronExpr: '0 * * * *', workflowId: 'wf.brief' });
  ...
  it('a personal schedule cannot also bind a rosterId; anon owner=me is refused', async () => {
    const c = client();
    await signup(c);
    const bad = await c.post('/v1/host/sample/scheduler/jobs', { owner: 'me', cronExpr: '0 9 * * *', rosterId: 'host:sally-1' });
    expect(bad.status).toBe(400);

    const anon = client();
    const r = await anon.post('/v1/host/sample/scheduler/jobs', { owner: 'me', cronExpr: '0 9 * * *' });
    expect(r.status).toBe(401);
  });
});
