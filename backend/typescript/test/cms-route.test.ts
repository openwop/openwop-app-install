...
async function signup(c: Client): Promise<{ userId: string }> {
  const r = await c.post('/v1/host/sample/users/auth/signup', { email: uniqEmail('cms'), password: 'password123' });
...
describe('cms — toggle gating', () => {
  it('404s when the cms toggle is off', async () => {
    await enableCms('off');
    const { owner, orgId } = await ownerWithMember('viewer');
    expect((await owner.get(u(orgId, '/pages'))).status).toBe(404);
    await enableCms('on');
  });
});
