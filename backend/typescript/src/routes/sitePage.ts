 ...
 *
 *   GET /v1/host/sample/site-page   the system home page (working copy)  [superadmin]
 *   PUT /v1/host/sample/site-page   edit title/sections + re-publish     [superadmin]
 *
 * The public render is served by the existing public Publishing surface
 * (`GET /v1/host/sample/public/host-site/pages/home`) — published-only, unchanged.
 */
...
export function registerSitePageRoutes(app: Express): void {
  app.get('/v1/host/sample/site-page', async (req, res, next) => {
    try {
  ...
  app.put('/v1/host/sample/site-page', async (req, res, next) => {
    try {
      requireSuperadmin(req, 'Home-page editing');
