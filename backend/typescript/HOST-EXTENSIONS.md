...
| 4. Seed | same file | Idempotent + per-entity (insert only what's missing → re-run is a no-op and a partial seed self-heals). Register it in `src/host/seedEverything.ts` `EXAMPLE_SEED_DOMAINS` + `verifyDomains` so the first-load seed stays provably comprehensive |
| 5. Routes | `src/routes/<your>.ts` | Vendor-prefixed namespace (`/v1/host/openwop-app/...`), `tenantOf(req)` accessor, `ok: false` → **409 + machine-readable `reason`**, validation → `OpenwopError('validation_error', …, 400)`, everything else → `next(err)` (the canonical envelope middleware) |
